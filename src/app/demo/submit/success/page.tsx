...
export default function SuccessPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch both ticket and queue data in parallel
        const [ticketData, queueData] = await Promise.all([
          dataProvider.getTicketByRef(referenceNumber),
          dataProvider.getQueueSnapshot("alejandro"), // Hardcoded for demo
        ]);
  ...
  const ticketData = {
    ...
    minPriorityTipCents: 1500, // Mock for now - should come from creator data
    queueMetrics: {
